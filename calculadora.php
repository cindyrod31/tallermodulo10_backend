...
<a href="index.html#section-respuestas">   
    ...
    if (isset($_POST['operar'])) {
        ...
        if (empty($n1) && empty($n2) && in_array($tipo, ['1', '2', '3', '4'])) {
            // Si ambos campos están vacíos y la operación es suma, resta, multiplicación o división
            echo "<script>alert('Debe ingresar valores en las casillas Número 1 y Número 2 para realizar la operación.');</script>";
        } elseif (empty($n1) && in_array($tipo, ['1', '2', '3', '4'])) {
            // Si el campo Número 1 está vacío y la operación es suma, resta, multiplicación o división
            echo "<script>alert('Debe ingresar un valor en la casilla Número 1 para realizar la operación.');</script>";
        } elseif (empty($n2) && $tipo !== '5') {
            // Si el campo Número 2 está vacío y la operación no es la raíz cuadrada
            ...
            switch ($tipo) {
                ...
                case '5':
                    if (!empty($n2)) {
                        // Si se ingresó un valor en el campo Número 2 para la operación de raíz cuadrada
                        echo "<script>alert('Para calcular la raíz cuadrada, debe ingresar el valor únicamente en la casilla Número 1.');</script>";
                    } else {
